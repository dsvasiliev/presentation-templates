...
for(const file_place of file_places) {
    // События для блока
    file_place.addEventListener('click', file_place_click)
...
function send_file() {
    ...
    // Проверка выбора местоположения
    ...
    if (first_file === undefined) return input_error(file_places[0]);
    if (second_file === undefined) return input_error(file_places[1]);
    ...
    $.ajax({
        ...
        success: function(answer){
            console.log(answer)
            if(answer.success) {
                TransitionPro.close(get_my_elems(), '/template?&id='+answer.id)
            }
            else {
                return newWarn.open(answer.message, 3000)
            }
        }
    });
}